...
class AccountView (FormView):    
    ...
    def post (self, request):
        ...
        if "submit_transaction" in request.POST and transaction_form.is_valid():
            ...
            current_user = request.user #!!Should it be declared before logical statements?
            current_category = CatModel.objects.filter(
                category_id=SelectedCat.objects.filter(user=current_user)[0].selected_category)[0] # Throws 'get() returned more than one Category -- it returned 2!'.
